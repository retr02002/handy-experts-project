...
export default function VendorLiveCallsPage() {
  // Simulating filtering calls assigned to this specific vendor's technicians
  // In a real app, we would fetch calls by vendor ID. For now we just show in_progress and assigned.
    ...
    <div className="flex flex-col gap-6 w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Monitor your technicians&apos; active service calls in real-time.</p>
        </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-slate-800 rounded-2xl p-4 min-h-[500px] flex flex-col shadow-sm relative overflow-hidden">
          <div className="absolute inset-0 bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
            ...
            <div className="text-center opacity-50">
              ...
              <p className="text-slate-500 font-medium">Interactive Map Interface</p>
            </div>
          </div>
          <div className="absolute inset-0 bg-slate-900/5 dark:bg-[#0F172A]/20"></div>
          
          <div className="relative z-10 flex items-center justify-between bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm p-3 rounded-xl border border-slate-200/50 dark:border-slate-700/50 mb-auto mx-2 mt-2 shadow-sm">
            <span className="text-sm font-semibold text-slate-900 dark:text-white flex items-center gap-2">
              <ClientIcon icon="ph:map-pin" className="w-4 h-4 text-emerald-500" />
              Technician Locations
            </span>
            <span className="text-xs font-medium text-slate-500 dark:text-slate-400">{liveCalls.length} Active Jobs</span>
          </div>
          
          {/* Mock map markers */}
          <div className="absolute top-1/3 left-1/4 w-10 h-10 bg-blue-500 text-white rounded-full border-2 border-white shadow-lg flex items-center justify-center animate-bounce z-10">
            <ClientIcon icon="ph:user" className="w-5 h-5" />
          </div>
          <div className="absolute top-1/2 left-1/2 w-10 h-10 bg-emerald-500 text-white rounded-full border-2 border-white shadow-lg flex items-center justify-center z-10">
            <ClientIcon icon="ph:user" className="w-5 h-5" />
          </div>
        </div>
          ...
          <div className="flex flex-col gap-3 overflow-y-auto max-h-[500px] pr-2">
            {liveCalls.length === 0 ? (
              <div className="p-6 text-center text-slate-500 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800">
                No active calls for your technicians right now.
              </div>
            ) : (
              liveCalls.map(call => (
                <ServiceCallCard 
