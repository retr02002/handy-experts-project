...
import { AddFreelanceTechnicianButton } from "./AddFreelanceTechnicianButton";

export default async function FreelanceTechniciansPage() {
  const res = await getAllTechniciansForAdminAction();
  const allTechnicians = res.success && res.data ? res.data : [];
  const technicians = allTechnicians.filter((t) => t.type === "FREELANCE");
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Freelance Technicians</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Technicians managed directly by Super Admin.
          </p>
        </div>
        <AddFreelanceTechnicianButton />
      </div>
      ...
      {technicians.length === 0 ? (
        <div className="bg-white dark:bg-[#0F172A] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-12 text-center">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">No freelance technicians registered yet.</p>
        </div>
      ) : (
            ...
            <table className="w-full min-w-[860px]">
              <thead className="bg-slate-50 dark:bg-slate-800/50">
                <tr className="text-left">
                  {["Technician", "Skill", "Rating", "Completed", "Active", "Status"].map((h) => (
                ...
                {technicians.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="p-4 min-w-0">
                      <Link href={`/admin/freelance-technicians/${t.id}`} className="block">
