...
type Props = {
  areas: TechnicianServiceAreaSummary[];
};

export function ServiceAreasManager({ areas }: Props) {
  const router = useRouter();
  const [modalOpen, setModalOpen] = useState(false);
  const [deleting, setDeleting] = useState<TechnicianServiceAreaSummary | null>(null);
  const [isDeleting, setIsDeleting] = useState(false);
  const [savingId, setSavingId] = useState<string | null>(null);
  const [radiusDrafts, setRadiusDrafts] = useState<Record<string, number>>({});

  const draftFor = (area: TechnicianServiceAreaSummary) => radiusDrafts[area.id] ?? area.radiusKm;

  const handleSaveRadius = async (area: TechnicianServiceAreaSummary) => {
    ...
    <div className="flex flex-col gap-4 sm:gap-6 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl p-3 sm:p-5 rounded-2xl border border-slate-200/60 dark:border-slate-800 shadow-sm">
        <div className="flex items-center sm:items-start gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center shrink-0 shadow-sm shadow-amber-500/20">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5 max-w-lg leading-snug">
              Define the regions where you work — visible to your vendor and admins on the live-calls map.
            </p>
          </div>
        </div>
        <button
          onClick={() => setModalOpen(true)}
          className="group relative flex items-center justify-center gap-2 px-4 py-2 sm:py-2.5 bg-slate-900 hover:bg-slate-800 dark:bg-amber-600 dark:hover:bg-amber-500 text-white rounded-xl text-sm font-bold transition-all hover:-translate-y-0.5 active:translate-y-0 shrink-0 cursor-pointer overflow-hidden shadow-sm hover:shadow-md dark:shadow-amber-500/10 w-full sm:w-auto"
        >
      ...
      {areas.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 px-6 gap-3 bg-white/40 dark:bg-slate-900/40 backdrop-blur-sm rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 relative overflow-hidden group">
          <div className="absolute inset-0 bg-gradient-to-b from-amber-50/50 to-transparent dark:from-amber-900/10 dark:to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          <div className="w-20 h-20 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center relative z-10 group-hover:scale-110 transition-transform duration-500">
            <ClientIcon icon="ph:map-pin-area-duotone" className="w-10 h-10 text-slate-400 group-hover:text-amber-500 transition-colors duration-300" />
          </div>
          <div className="text-center relative z-10">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">No areas defined</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 max-w-sm">Add your first service area so your vendor and admins can see where you work.</p>
          </div>
          <button
            onClick={() => setModalOpen(true)}
            className="mt-2 text-sm font-bold text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 relative z-10 flex items-center gap-1 group/btn"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
          {areas.map((area, idx) => {
            ...
            return (
              ...
              >
                <div className="absolute top-0 left-0 w-full h-[3px] bg-gradient-to-r from-amber-500 to-orange-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

                <div className="flex items-start justify-between gap-2 relative z-10">
                  <div className="flex items-center gap-2.5">
                    <div className="w-9 h-9 rounded-lg bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center shrink-0 border border-amber-100/50 dark:border-amber-500/20 group-hover:scale-105 transition-transform duration-300">
                      <ClientIcon icon="ph:map-pin-area-fill" className="w-4 h-4 text-amber-600 dark:text-amber-400" />
                    </div>
                ...
                <div className="flex flex-col gap-1.5 px-2.5 py-2 rounded-lg bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800/80">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs font-black text-amber-600 dark:text-amber-400 bg-amber-100/50 dark:bg-amber-900/30 px-1.5 py-0.5 rounded">
                      {draft} km
                    </span>
                  </div>
                  ...
                  <div className="relative pt-1 pb-0.5">
                    <input
                      ...
                      className="w-full h-1 bg-slate-200 dark:bg-slate-700 rounded-full appearance-none cursor-pointer accent-amber-600 dark:accent-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/40"
                    />
                ...
                {dirty && (
                  <div className="transition-all duration-300 animate-in fade-in slide-in-from-top-2">
                    <button
                      ...
                      className="w-full h-8 rounded-lg bg-amber-600 hover:bg-amber-500 dark:bg-amber-600 dark:hover:bg-amber-500 text-white text-xs font-bold transition-colors cursor-pointer disabled:opacity-60 flex items-center justify-center gap-1.5 shadow-sm"
                    >
                      {savingId === area.id ? (
                        <>
      ...
      <ConfirmDeleteDialog
        ...
        description={`Removing "${deleting?.pincode}" takes it off the coverage map your vendor and admins see. This can't be undone.`}
