...
export function VideoUploadField({ label, value, onChange, error }: Props) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">{label} (optional)</label>
      ...
      {mode === "upload" ? (
        ...
        >
          ...
          <span className="text-sm text-slate-500 dark:text-slate-400">
            {isUploading ? "Uploading..." : "Click to choose a video (MP4, WEBM, MOV — max 50MB)"}
          </span>
        </div>
      ) : (
        <input
      ...
      {value && (
        <div className="relative w-full h-40 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-black">
