...
export function TechnicianInfoTab({ technician, onChanged }: { technician: VendorTechnician; onChanged: () => void }) {
  ...
  const [skillAssignments, setSkillAssignments] = useState<SkillAssignmentInput[]>(
    technician.skillAssignments.map((a) => ({ categoryId: a.categoryId, serviceIds: a.serviceIds }))
  );
  ...
  if (resetPassword) {
    return (
      <div className="bg-white dark:bg-[#0F172A] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-5 flex flex-col gap-4 max-w-md">
  ...
  return (
    <div className="bg-white dark:bg-[#0F172A] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-5 flex flex-col gap-4 max-w-2xl">
      {/* Read-only: the login identity, recoverable at any time. */}
      {!editing && (
        <div className="rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 p-3">
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Login Username</p>
          {technician.username ? (
            <div className="flex items-center gap-2">
              <code className="flex-1 text-sm font-mono font-bold text-slate-900 dark:text-white break-all">{technician.username}</code>
              <button
                type="button"
                onClick={() => {
                  navigator.clipboard.writeText(technician.username ?? "");
                  toast.success("Username copied");
                }}
                className="shrink-0 w-8 h-8 rounded-lg bg-[#00B4FF] text-white flex items-center justify-center hover:bg-[#0096fa] transition-colors cursor-pointer"
              >
                <ClientIcon icon="ph:copy-bold" className="w-3.5 h-3.5" />
              </button>
            </div>
          ) : (
            <p className="text-sm text-slate-400">Not set — they can sign in with their mobile number.</p>
          )}
        </div>
      )}
      ...
      {editing ? (
        ...
        <div>
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">What they do</p>
      ...
      <Field label="Experience (yrs)" type="number" value={form.experienceYears} onChange={(v) => set("experienceYears", v)} editing={editing} />
      <Field
      ...
      {confirmingDelete ? (
        <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded-xl p-4 flex flex-col gap-3">
          <p className="text-sm text-red-700 dark:text-red-400">Delete {technician.name}? This can&apos;t be undone.</p>
          ...
          {editing ? (
            ...
            <div className="flex gap-2 flex-wrap">
              <button
                ...
                className="flex-1 min-w-[160px] h-11 rounded-xl bg-[#00B4FF] hover:bg-[#0096fa] text-white text-sm font-bold flex items-center justify-center gap-2 cursor-pointer"
              >
              ...
              <button
                ...
                className="flex-1 min-w-[160px] h-11 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-sm font-semibold flex items-center justify-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-800/60 cursor-pointer disabled:opacity-60"
              >
              ...
              <button
                ...
                className="flex-1 min-w-[160px] h-11 rounded-xl border border-red-200 dark:border-red-500/30 text-red-600 dark:text-red-400 text-sm font-semibold flex items-center justify-center gap-2 hover:bg-red-50 dark:hover:bg-red-500/10 cursor-pointer"
              >
