...
interface LiveCallCardProps {
  ...
  onBuy?: (call: NearbyLiveCall) => void;
}

/**
 * Every field here is masked server-side (see NearbyLiveCall) — the
 * customer's full name, real phone, address, and payment details are only
 * ever handed to the client once the lead is bought (buyLiveCallAction's own
 * response), so there is nothing to "expand" or "hide" on this card.
 */
export function LiveCallCard({ call, isSelected, onSelect, onBuy }: LiveCallCardProps) {
    ...
    >
      ...
      <div className="flex flex-col gap-1 text-sm text-slate-600 dark:text-slate-400">
        <div className="flex items-center gap-1.5">
          ...
          {call.customerFirstName} &middot; {call.customerPhoneMasked}
        </div>
      ...
      {onBuy && (
        <button
          ...
          className="w-full h-9 rounded-lg bg-[#00B4FF] hover:bg-[#0096fa] text-white text-xs font-bold transition-colors cursor-pointer flex items-center justify-center gap-1.5"
        >
          <ClientIcon icon="ph:lock-key-fill" className="w-3.5 h-3.5" />
          Buy Call — ₹{call.leadPrice}
        </button>
      )}
    </div>
  );
