...
export function VendorNavbar({ isMobileMenuOpen, setMobileMenuOpen }: VendorNavbarProps) {
    ...
    <header className="h-16 bg-white dark:bg-[#0B1120] border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 md:px-6 sticky top-0 z-40 transition-colors">
      ...
      <div className="flex items-center gap-2 md:gap-3">
        ...
        <div className="flex items-center gap-0.5 md:gap-1 bg-slate-100/80 dark:bg-slate-800/80 backdrop-blur-sm p-1 rounded-full border border-slate-200/50 dark:border-slate-700/50">
          ...
          {/* Notifications */}
          <button className="relative p-1.5 md:p-2 text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white rounded-full transition-all hover:shadow-sm">
          ...
          {/* Wallet Link */}
          <Link href="/vendor/wallet" className="relative p-1.5 md:p-2 text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700 hover:text-indigo-500 rounded-full transition-all hover:shadow-sm">
          ...
          {isProfileOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 py-1 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="px-4 py-2 border-b border-slate-100 dark:border-slate-700">
                <p className="text-sm font-semibold text-slate-900 dark:text-white">Vendor User</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate">vendor@handyexperts.com</p>
              </div>
              <div className="p-1">
                <Link 
                  href="/vendor" 
