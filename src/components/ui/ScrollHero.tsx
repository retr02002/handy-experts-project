...
export function ScrollHero() {
  return (
    <div className="relative min-h-[100dvh] flex flex-col justify-center w-full overflow-hidden bg-white dark:bg-[#0f172a]">
      
      {/* Video Background */}
      <video
        src="/videos/hero-loop.mp4"
        className="absolute inset-0 w-full h-full object-cover opacity-30 dark:opacity-40 mix-blend-luminosity dark:mix-blend-normal"
        autoPlay
        loop
        muted
        playsInline
      ...
      {/* Radial Gradient Overlay to fade edges to solid color like the screenshot */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,rgba(255,255,255,0.9)_100%)] dark:bg-[radial-gradient(circle_at_center,transparent_0%,rgba(15,23,42,0.95)_100%)]" />
      
      {/* Overlay Grid lines (Subtle) */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#0000000a_1px,transparent_1px),linear-gradient(to_bottom,#0000000a_1px,transparent_1px)] dark:bg-[linear-gradient(to_right,#ffffff0a_1px,transparent_1px),linear-gradient(to_bottom,#ffffff0a_1px,transparent_1px)] bg-[size:4rem_4rem]" />
      ...
      <div className="relative z-10 flex flex-col items-center justify-center px-4 py-24 sm:py-32 w-full mt-10">
        ...
        <div className="flex flex-col items-center justify-center px-1 sm:px-4 max-w-4xl mx-auto w-full">
          <div className="inline-flex items-center space-x-2 bg-white/80 dark:bg-white/10 backdrop-blur-md rounded-full px-3 py-1 sm:px-4 sm:py-1.5 mb-4 text-[10px] sm:text-xs font-semibold text-slate-700 dark:text-white border border-slate-200 dark:border-white/20 shadow-sm">
            <ClientIcon icon="ph:sparkle-fill" className="text-[#00B4FF]" />
            <span>{TEXT_CONTENT.badge}</span>
          </div>
          
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold text-center tracking-tight mb-3 sm:mb-4 px-2">
            <span className="text-slate-900 dark:text-white drop-shadow-sm">{TEXT_CONTENT.title}</span> <br className="hidden sm:block" />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00B4FF] to-[#0070FF] drop-shadow-sm">{TEXT_CONTENT.titleHighlight}</span>
          </h1>
          
          <p className="text-sm sm:text-lg md:text-xl text-slate-600 dark:text-slate-200 text-center max-w-2xl mb-8 sm:mb-10 px-2 font-medium leading-relaxed drop-shadow-sm">
            {TEXT_CONTENT.description}
          </p>
          
          {/* Fresh UI Concept: Unified Search Command Bar */}
          <div className="w-full relative flex items-center bg-white/90 dark:bg-slate-900/80 backdrop-blur-2xl rounded-full p-1.5 sm:p-2 border border-slate-200/80 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.08)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] mx-auto max-w-2xl transition-all focus-within:ring-2 focus-within:ring-[#00B4FF]/40 focus-within:shadow-[0_8px_40px_rgb(0,180,255,0.15)] hover:-translate-y-0.5">
          ...
          {/* Features List */}
          <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-6 mt-8 sm:mt-10 text-[11px] sm:text-sm font-semibold text-slate-600 dark:text-white/90">
